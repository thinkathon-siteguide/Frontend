...
const Layout: React.FC<LayoutProps> = ({ children }) => {
  ...
  return (
    <div className="flex h-screen bg-gray-50 font-sans overflow-hidden">
      ...
      {/* Sidebar for Desktop */}
      <aside
        className={`fixed inset-y-0 left-0 z-50 w-72 bg-thinklab-black text-white shadow-2xl transform transition-transform duration-500 cubic-bezier(0.4, 0, 0.2, 1) md:translate-x-0 md:static md:inset-auto ${
                ...
                >
                  <item.icon
                    className={`w-5 h-5 mr-3 transition-colors ${
                      isActive
                        ? 'text-white'
                        : 'text-gray-500 group-hover:text-thinklab-red'
                    }`}
                  />
                  <span
      ...
      <div className="flex-1 flex flex-col overflow-hidden bg-gray-50">
        ...
        <header className="h-16 bg-white shadow-sm flex items-center justify-between px-6 z-30 border-b border-gray-200">
          <div className="flex items-center">
            <button
              onClick={toggleSidebar}
              className="md:hidden text-thinklab-black mr-4"
            >
          ...
          <div className="flex items-center ml-auto space-x-4">
            <div className="text-right hidden sm:block">
              <p className="text-sm font-bold text-thinklab-black">
                {user?.name || 'Engineer'}
              </p>
