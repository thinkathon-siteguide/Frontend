...
import { FileText, Download, Printer, Share2, Briefcase, ChevronDown, Loader2, Calendar, AlertTriangle, CheckCircle, BrainCircuit, AlertCircle, Edit3, Settings, MoreVertical, PieChart, TrendingUp, DollarSign } from 'lucide-react';
...
const Reports: React.FC = () => {
  ...
  const handleExport = () => {
    alert("Report exported to PDF (Demo)");
  };
  ...
  if (workspaces.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[70vh] text-center p-8 animate-fade-in-up">
         ...
         <h2 className="text-2xl font-bold text-thinklab-black">Report Builder</h2>
         <p className="text-thinklab-grey mt-2 mb-8 max-w-md">
            No projects found. Create a workspace to start generating automated AI progress reports.
         </p>
         <button 
  ...
  return (
    <div className="h-[calc(100vh-100px)] flex flex-col lg:flex-row gap-6 animate-fade-in overflow-hidden">
      ...
      <div className="w-full lg:w-80 flex flex-col gap-6 flex-shrink-0 bg-white p-6 rounded-3xl shadow-soft border border-gray-100 overflow-y-auto">
          <div>
            <h2 className="text-xl font-bold text-thinklab-black mb-1">Report Builder</h2>
          ...
          <div className="space-y-4">
              <div>
                  <label className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-2 block">Select Project</label>
              ...
              {activeWorkspace && (
                 <div className="bg-gray-50 p-4 rounded-xl border border-gray-100 space-y-3">
                     <div className="flex justify-between items-center text-sm">
                         ...
                         <span className="font-bold text-thinklab-black px-2 py-0.5 bg-white border border-gray-200 rounded text-xs">{activeWorkspace.status}</span>
                     </div>
                     <div className="flex justify-between items-center text-sm">
                         <span className="text-gray-500">Stage</span>
                         <span className="font-bold text-thinklab-black text-right truncate max-w-[120px]">{activeWorkspace.stage}</span>
                     </div>
                 </div>
              )}
          </div>
          ...
          <div className="space-y-3">
             ...
             > 
                ...
                <span className="relative z-10">{loading ? 'Generating...' : 'Generate New Report'}</span>
             </button>
          ...
          {!report && activeWorkspace && (
              <div className="mt-auto bg-blue-50 p-4 rounded-xl text-xs text-blue-700 leading-relaxed border border-blue-100">
                  ...
                  Generate a report at the end of each work day to maintain a comprehensive audit trail for your client.
              </div>
          )}
      </div>
      ...
      <div className="flex-1 overflow-y-auto bg-gray-100/50 rounded-3xl p-4 lg:p-8 flex justify-center items-start shadow-inner border border-gray-200/50 scrollbar-hide">
         {!activeWorkspace ? (
             ...
             <div className="flex flex-col items-center justify-center h-full max-w-md text-center">
                 ...
                 <h3 className="text-xl font-bold text-thinklab-black">Ready to Report</h3>
                 <p className="text-gray-400 mt-2 mb-8">
                    Our AI will analyze your project's inventory, safety logs, and timeline to draft a complete daily summary.
                 </p>
             </div>
         ) : loading ? (
             <div className="flex flex-col items-center justify-center h-full">
                 ...
                 <p className="text-thinklab-grey font-medium animate-pulse">Drafting report content...</p>
             </div>
         ) : report ? (
             <div className="bg-white w-full max-w-[800px] shadow-2xl min-h-[1000px] p-8 md:p-12 relative animate-fade-in-up">
                 ...
                 <header className="flex justify-between items-start border-b-2 border-thinklab-black pb-8 mb-8">
                     <div>
                         <div className="flex items-center gap-2 mb-4">
                            ...
                            <span className="font-bold text-lg tracking-tight">ThinkLab Group</span>
                         </div>
                         <h1 className="text-4xl font-serif font-bold text-thinklab-black mb-2">DAILY SITE REPORT</h1>
                     ...
                     <div className="text-right">
                         <div className="mb-2">
                             ...
                             <div className="font-serif font-bold text-lg">{report.date}</div>
                         </div>
                         <div>
                             <div className="text-xs text-gray-400 uppercase font-bold">Project Ref</div>
                             <div className="font-mono text-sm">{activeWorkspace.id.substring(0,8).toUpperCase()}</div>
                         </div>
                     </div>
                 </header>
                 ...
                 <div className="space-y-8 font-serif">
                     <section>
                         <h3 className="text-xs font-bold font-sans uppercase tracking-widest text-thinklab-red mb-3 flex items-center gap-2">
                             <div className="w-8 h-[2px] bg-thinklab-red"></div> Executive Summary
                         </h3>
                         <p className={`text-gray-800 leading-relaxed text-justify ${animateText ? 'animate-pulse' : ''}`}>
                     ...
                     <div className="grid grid-cols-2 gap-8 py-4 border-y border-gray-100 my-6 bg-gray-50/50 p-6 rounded-lg font-sans">
                         <div>
                            <span className="text-xs font-bold text-gray-400 uppercase block mb-1">Project Name</span>
                            <span className="font-bold text-thinklab-black block">{activeWorkspace.name}</span>
                         </div>
                         <div>
                            <span className="text-xs font-bold text-gray-400 uppercase block mb-1">Location</span>
                            <span className="font-bold text-thinklab-black block">{activeWorkspace.location}</span>
                         </div>
                         <div>
                            <span className="text-xs font-bold text-gray-400 uppercase block mb-1">Supervisor</span>
                            <span className="font-bold text-thinklab-black block">Farouq Oduola</span>
                         </div>
                         <div>
                            <span className="text-xs font-bold text-gray-400 uppercase block mb-1">Weather Conditions</span>
                            <span className="font-bold text-thinklab-black block">Sunny, 28°C</span>
                         </div>
                     </div>
                        ...
                        <section>
                            ...
                            <ul className="space-y-3">
                                {report.keyIssues.length > 0 ? report.keyIssues.map((issue, idx) => (
                                    <li key={idx} className="flex items-start text-sm text-gray-700 bg-red-50 p-3 rounded border-l-2 border-red-200">
                                        <span className="font-bold mr-2 text-red-500">{idx+1}.</span> {issue}
                                    </li>
                                )) : <li className="text-sm text-gray-400 italic">No significant issues logged today.</li>}
                            </ul>
                        </section>
                        ...
                        <section>
                            ...
                            <ul className="space-y-3">
                                {report.recommendations.map((rec, idx) => (
                                    <li key={idx} className="flex items-start text-sm text-gray-700 bg-green-50 p-3 rounded border-l-2 border-green-200">
                                        <span className="font-bold mr-2 text-green-600">✓</span> {rec}
                                    </li>
                                ))}
                            </ul>
                        </section>
             ...
             <>
                ...
                <div className="bg-white rounded-2xl p-5 shadow-soft border border-gray-100">
                    <div className="flex items-center justify-between mb-4">
                        <h4 className="font-bold text-thinklab-black text-sm">Budget Burn</h4>
                    ...
                    <div className="w-full bg-gray-100 rounded-full h-1.5 mt-2 mb-2">
                        <div className="bg-thinklab-black h-1.5 rounded-full" style={{width: '65%'}}></div>
                    </div>
                ...
                <div className="bg-white rounded-2xl p-5 shadow-soft border border-gray-100 relative overflow-hidden group">
                    ...
                    <div className="flex items-center justify-between mb-4 relative z-10">
                        <h4 className="font-bold text-thinklab-black text-sm">Safety Score</h4>
                    ...
                    <p className="text-xs text-gray-400 relative z-10">Based on last 7 days</p>
                </div>
                ...
                <div className="bg-thinklab-black rounded-2xl p-5 shadow-soft text-white">
                    ...
                    <div className="flex items-end gap-2">
                        <span className="text-4xl font-bold">{activeWorkspace.progress}%</span>
                    </div>
                    <div className="mt-4 flex gap-2">
                        {[1,2,3,4,5].map(i => (
                            <div key={i} className={`h-1 flex-1 rounded-full ${i*20 <= activeWorkspace.progress ? 'bg-yellow-400' : 'bg-gray-700'}`}></div>
                        ))}
                    </div>
                </div>
             </>
