...
const WorkspaceManagement: React.FC = () => {
  ...
  const openEditModal = (e: React.MouseEvent, ws: any) => {
    e.stopPropagation();
    setIsEditMode(true);
    setEditingId(ws._id);
    setFormData({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (isEditMode && editingId) {
      updateWorkspace(editingId, formData);
    } else {
  ...
  const filteredWorkspaces = workspaces.filter(ws => {
    ...
    const matchesFilter = filterType === 'All' || ws.status === filterType || 
                          (filterType === 'Active' && ws.status === 'Under Construction');
  ...
  const portfolioStats = {
    ...
    avgProgress: workspaces.length > 0 
      ? Math.round(workspaces.reduce((acc, cur) => acc + cur.progress, 0) / workspaces.length) 
      : 0,
  };
      ...
      <div className="flex flex-col gap-6">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <button 
            ...
            disabled={isCreating}
            className="bg-thinklab-black text-white px-6 py-3 rounded-xl font-bold flex items-center hover:bg-gray-800 transition shadow-lg group disabled:opacity-50"
          >
        ...
        <div className={viewMode === 'grid' ? "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6" : "space-y-4"}>
          {filteredWorkspaces.map((ws, index) => (
            viewMode === 'grid' ? (
              <div 
                key={ws._id} 
                className="bg-white rounded-2xl shadow-soft border border-gray-100 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group overflow-hidden flex flex-col animate-fade-in-up cursor-pointer"
                style={{ animationDelay: `${index * 50}ms` }}
                onClick={() => { setActiveWorkspace(ws._id); navigate('/'); }}
              >
                ...
                <div className="p-6 flex-1 flex flex-col">
                  <div className="flex justify-between items-start mb-2">
                    <div>
                      <h3 className="text-lg font-bold text-thinklab-black group-hover:text-thinklab-red transition-colors line-clamp-1">
                        {ws.name}
                      </h3>
                  ...
                  <div className="mt-4 pt-4 border-t border-gray-50 space-y-4">
                    <div>
                      ...
                      <div onClick={(e) => e.stopPropagation()}>
                        <input 
                          ...
                          max="100" 
                          value={ws.progress} 
                          onChange={(e) => updateProgress(ws._id, parseInt(e.target.value))}
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex gap-2">
                        ...
                        <button 
                          onClick={(e) => { e.stopPropagation(); setActiveWorkspace(ws._id); navigate('/'); }}
              ...
              <div 
                key={ws._id} 
                className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 hover:border-gray-200 transition-all flex items-center justify-between group animate-fade-in-up cursor-pointer"
                style={{ animationDelay: `${index * 50}ms` }}
                onClick={() => { setActiveWorkspace(ws._id); navigate('/'); }}
              >
                <div className="flex items-center gap-4 flex-1">
                  ...
                  <div>
                    <h3 className="font-bold text-thinklab-black group-hover:text-thinklab-red transition-colors">{ws.name}</h3>
                ...
                <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                  ...
                  <button onClick={(e) => handleDelete(e, ws._id)} className="p-2 hover:bg-red-50 rounded text-gray-500 hover:text-red-600">
                    <Trash2 className="w-4 h-4" />
                  </button>
                </div>
              </div>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-5">
              ...
              <div className="pt-6 flex gap-3 border-t border-gray-100 mt-2">
                ...
                <button 
                  type="submit" 
                  disabled={isCreating}
                  className="flex-1 py-3 bg-thinklab-black text-white rounded-xl font-bold hover:bg-thinklab-red transition-colors shadow-lg shadow-gray-200 flex justify-center items-center disabled:opacity-50"
                >
